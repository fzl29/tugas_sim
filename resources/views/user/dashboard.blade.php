...
<section class="grid grid-cols-3 gap-4 mb-4">
    <div class="flex items-center gap-4 bg-light7 dark:bg-dark7 text-light1 dark:text-dark1 rounded-md border border-light5 dark:border-dark5 py-4 px-5">
        ...
        <div class="flex flex-col">
            ...
            <h2 class="text-3xl font-semibold">{{ $bukuDipinjam }}</h2>
        </div> 
    </div>
    <div class="flex items-center gap-4 bg-light7 dark:bg-dark7 text-red-500 fill-red-500 rounded-md border border-light5 dark:border-dark5 py-4 px-5">
        ...
        <div class="flex flex-col">
            ...
            <h2 class="text-3xl font-semibold">{{ $antrianAktif }}</h2>
        </div> 
    </div>
                ...
                <div class="w-full flex overflow-x-auto">
                    <div class="flex gap-5 w-max">
                        @foreach($bukuTerbaru as $book)
                            <div class="flex w-48 flex-col gap-3 p-3 rounded-md border border-light5 dark:border-dark5 h-full">
                                <img src="{{ asset('storage/'.$book->cover) }}"
                                    class="w-48 h-60 object-cover rounded-md" loading="lazy" />
                                <div class="flex flex-col h-full">
                                    <h4 class="font-semibold font-raleway text-[16px] leading-5 text-light3 dark:text-dark3 mb-1">
                                        {{ $book->title }}
                                    </h4>
                                    <p class="text-[15px] text-light4 dark:text-dark4">
                                        {{ $book->author }}
                                    </p>
                                    <span class="text-[14px] mt-auto
        ...
        <div class="bg-light7 dark:bg-dark7 rounded-md border border-light5 dark:border-dark5 p-6">
            ...
            <div>
                ...
                <h2 class="text-3xl font-bold text-orange-500">
                    {{ $queuePosition ? str_pad($queuePosition, 2, '0', STR_PAD_LEFT) : '--' }}
                </h2>
                <p class="text-[13px] {{ $queueStatus === 'menunggu' ? 'text-orange-500' : 'text-orange-500' }}">
                    {{ $queueStatus ? ucfirst($queueStatus) : 'Tidak ada antrian aktif' }}
                </p>
            </div>
        </div>
                ...
                <ul class="text-[15px] space-y-1">
                    @forelse($riwayatPeminjaman as $loan)
                        <li>
                            ...
                            <strong>Buku "{{ $loan->book->title ?? '-' }}" </strong>
                    ...
                    @empty
                        <li>Belum ada riwayat peminjaman.</li>
                    @endforelse
                </ul>
            </div>
        </div>
        ...
        <div class="p-6 h-fit rounded-md border bg-light7 dark:bg-dark7 border-light5 dark:border-dark5">
            ...
            <ul class="space-y-4 text-[15px]">
                ...
                <li class="flex flex-col"><strong>Catatan:</strong> Peminjaman buku hanya untuk antrian dan pengambilan secara offline tukar dengan nomor antrian anda.
                    <br> <br> <span class="text-red-500">*Jika telat mengembalikan Denda Rp 10.000</span>
                </li>
            </ul>
        </div>
