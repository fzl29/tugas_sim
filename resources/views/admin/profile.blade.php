...
<section class="px-6 pt-4 pb-6 rounded-md bg-light7 dark:bg-dark7 text-light4 dark:text-dark4 border border-light5 dark:border-dark5">
    ...
    <div id="content-default" class="px-1">
        ...
        <form id="profile-form" action="{{ route('admin.profile.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-[1fr_3.5fr] gap-5 items-center text-[15px]">
                <div class="flex flex-col items-center gap-3">
                    <img id="preview-avatar" src="{{ asset(Auth::user()->avatar) }}" 
                        alt="Foto Profil" 
                    ...
                    <button type="button" id="upload-button" 
                        class="cursor-pointer text-[13px] px-3 py-1.5 rounded-md bg-light5 text-light7 hover:bg-light1 dark:bg-dark5 dark:text-dark7 dark:hover:bg-dark1 transition-all duration-200 hidden">
                        Pilih Foto
                    </button>
                </div>
                <div>
                    <div class="grid grid-cols-2 gap-y-4 gap-x-5 text-[15px] mb-6">
                        @include('components.input-readonly', ['label' => 'Nama Lengkap', 'value' => $admin->name])
                        @include('components.input-readonly', ['label' => 'Username', 'value' => $admin->username])
                        @include('components.input-readonly', ['label' => 'NUPTK', 'value' => $admin->identifier])
                        @include('components.input-editable', ['label' => 'Email', 'name' => 'email', 'value' => $admin->email])
                        @include('components.input-editable', ['label' => 'No. HP', 'name' => 'phone', 'value' => $admin->phone])
                    </div>
        ...
        {{-- Alert Section --}}
        @if (session('success'))
            <div class="mt-5 px-4 py-3 rounded-md bg-green-100 border border-green-400 text-green-700">
                {{ session('success') }}
            </div>
        @endif

        @if ($errors->any())
            <div class="mt-5 px-4 py-3 rounded-md bg-red-100 border border-red-400 text-red-700">
                <ul>
    ...
    <div id="content-change" class="hidden mt-6 px-1">
        <form action="{{ route('admin.profile.password') }}" method="POST">
            ...
            <div class="grid grid-cols-2 gap-y-4 gap-x-5 text-[15px] mb-6">
                ...
                @include('components.input-password', ['label' => 'Confirm New Password', 'name' => 'password-new_confirmation', 'place' => 'Masukkan password baru kembali'])
            </div>
